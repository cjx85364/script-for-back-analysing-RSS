...
# coding: utf-8
# %%
# post earthquake landslide stable in coseismic phase. this code is used to simulate the shear strength of unfailed hillslope during earthquake 
# %%
...
# we defined "calculate_ky" to create random cohesion and friction angle, and use these values to calculate ac. From all ac values, we select the couples that suit failed condition (ac<pga) 

class co_landslide_strength_inversion():
    ...
    def calculate_ky(self, i):
        ...
        self._PGA = self.data['peak_ground_acceleration'][i]
        self._m=self.data['m_before'][i]
        ...
        # filtering cohesion and friction angle according to stable condition (ac>pga), and calculate mean
        self._ky=(self._FS-1)*np.sin(self._slope*np.pi/180.)#unit/g 
        self._mean_phi=np.mean(self._phi[self._ky>self._PGA])
        self._mean_C=np.mean(self._C[self._ky>self._PGA])
        # a try to find limit condition, useless in this study 
        if len([x for x in self._ky[self._ky>self._PGA]])>=1:
            self._li_phi=np.mean(self._phi[self._ky==min([x for x in self._ky[self._ky>self._PGA]])])
            self._li_C=np.mean(self._C[self._ky==min([x for x in self._ky[self._ky>self._PGA]])])
        else:
            self._li_phi=-9999.0
            self._li_C=-9999.0
